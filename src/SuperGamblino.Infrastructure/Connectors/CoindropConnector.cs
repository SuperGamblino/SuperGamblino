...
namespace SuperGamblino.Infrastructure.Connectors
{
    ...
    {
        public CoindropConnector(ILogger<CoindropConnector> logger, ConnectionString connectionString) : base(logger, connectionString)
        {
        }
        ...
        public virtual async Task<int> CollectCoinDrop(ulong channelId, int claimId)
        {
            ...
            try
            {
                var sql = new MySqlCommand(
                    $"SELECT claimed, coin_reward, claim_id FROM coin_drop WHERE channel_id = '{channelId}' ORDER BY id DESC LIMIT 0, 1 ",
                    c);
                await c.OpenAsync();
