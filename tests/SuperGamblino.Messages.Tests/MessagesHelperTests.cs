    ...
    public class MessagesHelperTests
    {
        ...
        public void IsInvalidArgumentsFormattedCorrectly()
        {
            var result = GetMessagesHelper()
                .InvalidArguments(new[] {"<HEAD|TAIL>", "<BET>"}, "example-command", "Example Title");

            Assert.Equal("Example Title", result.Title);
            Assert.Equal("Invalid arguments!\nUse the following example-command: <HEAD|TAIL> <BET>",
                result.Description);
            Assert.Equal("WARNING", result.Color);
            Assert.True(string.IsNullOrWhiteSpace(result.Footer));
            Assert.Empty(result.Fields);
        ...
        public void IsCommandCalledTooEarlyFormattedCorrectly()
        {
            ...
            Assert.Equal("Example Title", result.Title);
            Assert.Equal(
                "You've tried to execute command 'time-based-command' before it was ready! Command will be ready in 05:00:00",
                result.Description);
            Assert.Equal("WARNING", result.Color);
            Assert.True(string.IsNullOrWhiteSpace(result.Footer));
            Assert.Empty(result.Fields);
        ...
        {
            var result = GetMessagesHelper()
                .LoseInformation(100, "Example info about the losed game.", "Example Title");
        ...
        public void IsAddExpInformationWorkingCorrectlyWhenUserLeveledUp()
        {
            ...
            var result = messagesHelper.AddExpInformation(
                messagesHelper.Information("Example Information", "Example Title"),
                new AddExpResult(true, 100, 20, 300));
        ...
        public void IsAddExpInformationWorkingCorrectlyWhenUserDidNotLevelUp()
        {
            ...
            var result = messagesHelper.AddExpInformation(
                messagesHelper.Information("Example Information", "Example Title"),
                new AddExpResult(false, 100, 20, 300));
        ...
        public void IsAddCoinsBalanceInformationFormattedCorrectly()
        {
            ...
            var result =
                messageHelper.AddCoinsBalanceInformation(
                    messageHelper.Information("Example Information", "Example Title"), 100);
        ...
        {
            var messagesHelper = GetMessagesHelper();
            var result = messagesHelper.AddCoinsBalanceAndExpInformation(
                messagesHelper.Information("Example Information", "Example Title"),
                new AddExpResult(true, 100, 20, 300), 100);
        ...
        public void IsAddCoinsBalanceAndExpInformationBeingFormattedCorrectlyWhenUserDidNotLevelUp()
        {
            ...
            var result = messagesHelper.AddCoinsBalanceAndExpInformation(
                messagesHelper.Information("Example Information", "Example Title"),
                new AddExpResult(false, 100, 20, 300), 100);
        ...
        public void IsAddLevelUpMessageFormattedCorrectly()
        {
            var messageHelper = GetMessagesHelper();
            var result =
                messageHelper.AddLevelUpMessage(messageHelper.Information("Example informative information!",
                    "Example Title"));
